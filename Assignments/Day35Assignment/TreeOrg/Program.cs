namespace TreeOrg
{
    ...
    public class OrganizationTree
    {
        ...
        private void PrintRecursive(EmployeeNode current, int depth)
        {
            ...
            string s = "";
            Console.WriteLine(indent + s + current.Position);
            if (current.Reports.Count == 0) return;
            foreach(EmployeeNode employee in current.Reports)
            {
                PrintRecursive(employee, depth + 1);
            }
        }
    }
